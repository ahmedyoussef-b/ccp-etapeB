    ...
    (userMessage: string) => {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          const ragHistory = messages
            .filter((m) => m.role === "user" || m.role === "assistant")
            .slice(-6)
            .map((m) => `${m.role === "user" ? "Utilisateur" : "Assistant"}: ${m.content}`);
          const result = await orchestrator.answerQuery(userMessage, ragHistory);
          finishResponse(result.answer, result.mediaItems.length > 0 ? result.mediaItems : undefined);
          return;
        } catch {
          // RAG unavailable — fall through to legacy responses
        }
