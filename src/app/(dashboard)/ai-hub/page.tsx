...
import { createRagOrchestrator } from "@/lib/rag/orchestrator";
import {
  Send,
  Bot,
  Trash2,
...
Si un document est mentionné, extrait son id dans documentId, sinon null.`;
  ...
  const handleRAGResponse = useCallback(async (query: string): Promise<{ text: string; mediaItems?: Message["mediaItems"] }> => {
    try {
      const orchestrator = createRagOrchestrator();
      const result = await orchestrator.answerQuery(query);
      return {
        text: result.answer,
        mediaItems: result.mediaItems.length > 0 ? result.mediaItems : undefined,
      };
    } catch {
      const response = await chat(
        [
...
- Pas de texte hors du JSON`,
  ...
  const finishResponse = useCallback((response: string, mediaItems?: Message["mediaItems"]) => {
    setMessages((prev) => [
      ...prev,
      {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (intention === "qa") {
        const ragResult = await handleRAGResponse(trimmed);
        finishResponse(ragResult.text, ragResult.mediaItems);
        return;
      }
                    ...
                    >
                      ...
                      <div className={`group flex flex-col ${isUser ? "items-end" : "items-start"} max-w-[85%] sm:max-w-[75%]`}>
                        ...
                        >
                          ...
                          {!isUser && message.mediaItems && message.mediaItems.length > 0 && (
                            <ChatMediaRenderer mediaItems={message.mediaItems} />
                          )}
                        </Card>
