...
export async function POST(request: Request): Promise<NextResponse<DeployResponse>> {
  ...
  try {
    ...
    // ============================================
    // PHASE 4: COMMIT AUTOMATIQUE (Git Add + Commit + Push)
    // ============================================
    ...
    try {
      ...
      logger.info('Phase 3 a deja uploade via l\'API GitHub, pas de reset local necessaire');
      ...
      const commitMessage = `Auto-deploy: Pipeline ${new Date().toISOString().replace(/[:.]/g, "-")}`;
      try {
        execSync(`git commit -n -m "${commitMessage}"`, { cwd: gitDir, stdio: "pipe" });
        logger.success("Commit local cree");
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        if (msg.includes("nothing to commit") || msg.includes("rien à commettre")) {
          logger.info("Rien à committer, passage au push");
        } else {
      ...
      try {
        execSync(`git push origin ${branch}`, { cwd: gitDir, stdio: "pipe" });
        logger.success("Push effectue sur la branche distante");
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        throw new Error(`git push failed: ${msg}`);
      }

      logger.success("Pipeline termine avec succes!");

    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
      logger.error(`Erreur lors du commit automatique: ${errorMessage}`);
      throw new Error("Echec critique de la Phase 4");
    }
