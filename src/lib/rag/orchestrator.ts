...
import { imageRetriever } from "./sources/image-retriever";
...
- "image" : recherche d'images, vidéos, médias dans la banque d'images, photos, vidéos, documents visuels
...
5. Si la question contient des mots comme "image", "photo", "vidéo", "visuel", "média", "banque d'images", "montre-moi", "affiche", utilise "image".
6. Si la question est générale ou ambiguë, choisis "local" par défaut.

Réponds UNIQUEMENT par un objet JSON valide :
{
  "primary": "local" | "vector" | "web" | "database" | "image",
  "secondary": ["local", "vector", "web", "database", "image"][] | null,
...
export class RagOrchestrator {
  ...
  async answerQuery(userQuery: string, chatHistory: string[] = []): Promise<RagResult> {
    ...
    const mediaItems = contexts.flatMap((ctx) => ctx.mediaItems || []);
  ...
  private async gatherContexts(query: string, decision: RouterDecision): Promise<RagContext[]> {
    ...
    const runRetriever = async (key: "local" | "vector" | "web" | "database" | "image") => {
      const retriever = retrievers[key];
      if (!retriever) return;
      try {
        const chunks = await retriever.retrieve(query, 5);
        if (chunks.length > 0) {
          const mediaItems = chunks.flatMap((c) => c.mediaItems || []);
          contexts.push({
    ...
    if (decision.secondary && decision.secondary.length > 0) {
      await Promise.all(decision.secondary.map((s) => runRetriever(s as "local" | "vector" | "web" | "database" | "image")));
    }
...
Exemple : [2, 0, 3]`;
  ...
  private fuseContexts(contexts: RagContext[]): string {
    ...
    const blocks = contexts.map((ctx) => {
      ...
      let block = `=== ${label} ===\n${ctx.content}`;
      if (ctx.mediaItems && ctx.mediaItems.length > 0) {
        block += "\n\nMÉDIAS ASSOCIÉS :";
        for (const media of ctx.mediaItems) {
          block += `\n- [MEDIA:${media.kind.toUpperCase()}:${media.id}] ${media.title} (${media.category})`;
        }
        block += "\n\nTu PEUX et DOIS afficher ces médias dans ta réponse en utilisant les balises [MEDIA:IMAGE:id] ou [MEDIA:VIDEO:id]. Par exemple : [MEDIA:IMAGE:media_123]";
      }
      return block;
    });
...
Cite toujours tes sources entre crochets, ex: [LOCAL], [VECTOR], [WEB], [DATABASE], [IMAGE].

IMPORTANT : Si le contexte contient des médias (images ou vidéos) de la banque d'images, tu DOIS les afficher dans ta réponse en utilisant les balises spéciales :
- Pour une image : [MEDIA:IMAGE:id]
- Pour une vidéo : [MEDIA:VIDEO:id]

Exemple de réponse avec média :
"Voici le plan du bloc B que vous demandez : [MEDIA:IMAGE:media_123456_abc]"
